...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/activities/activitiesSlice';
...
const TableSampleActivities = ({
  ...
  useEffect(() => {
    if (activitiesNotify.showNotification) {
      notify(
        activitiesNotify.typeNotification,
        activitiesNotify.textNotification,
      );
    }
  ...
  const handleViewAction = (id: string) => {
    router.push(`/activities/activities-view/?id=${id}`);
  };
