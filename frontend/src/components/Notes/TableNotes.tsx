...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/notes/notesSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureNotesCols';
...
const TableSampleNotes = ({
  ...
  useEffect(() => {
    if (notesNotify.showNotification) {
      notify(notesNotify.typeNotification, notesNotify.textNotification);
    }
  }, [notesNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/notes/notes-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {notes && Array.isArray(notes) && !showGrid && (
        <CardNotes
          notes={notes}
          loading={loading}
...
export default TableSampleNotes;
